...
public:
	...
	*/
		...
		std::string cust_type, std::string account_type)
	{
		...
		if (cust_type == "Senior") cust = new Senior(name, address, telephone, age);
		if (cust_type == "Adult") cust = new Adult(name, address, telephone, age);
		if (cust_type == "Student") cust = new Student(name, address, telephone, age);
