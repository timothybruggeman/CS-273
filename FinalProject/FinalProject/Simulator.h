...
public:
	...
	Simulator() {
		...
		rate_of_patients = read_int("What is the rate of new patients per hour at this hospital? ", 1, 59) / 60;
	...
	void show_stats() {
		...
		std::cout << "Number of total patients serves: " << visit_list.size() << std::endl;
		...
		for (int i = 0; i < visit_list.size(); i++) {
			std::cout << "Patient: " << visit_list[i].get_patient().get_first_name << " " << visit_list[i].get_patient().get_last_name
				...
				<< "     Helped by: " << visit_list[i].get_healthworker().get_last_name() << endl;
		}
	}
};
